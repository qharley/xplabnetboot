...
# ─────────────────────────────────────────────
# Sanity-check: verify nginx can serve the squashfs before clients try
# ─────────────────────────────────────────────
echo "==> Verifying HTTP access to squashfs..."
SQUASHFS_HTTP_STATUS="$(wget -q --spider --server-response \
    "${LIVE_SQUASHFS_URL}" 2>&1 | awk '/HTTP\//{print $2}' | tail -1)"
if [ "${SQUASHFS_HTTP_STATUS}" = "200" ]; then
    echo "    OK  ${LIVE_SQUASHFS_URL}"
else
    echo "    FAIL (HTTP ${SQUASHFS_HTTP_STATUS:-no response}) ${LIVE_SQUASHFS_URL}"
    echo "    Checking mount and file..."
    ISO_CONTENTS="${HTTP_ROOT}/iso-contents"
    mountpoint -q "${ISO_CONTENTS}" && echo "    ${ISO_CONTENTS} is mounted" \
        || echo "    WARNING: ${ISO_CONTENTS} is NOT mounted – loop-mount failed or was skipped"
    ls -lh "${ISO_CONTENTS}/live/" 2>/dev/null || echo "    ${ISO_CONTENTS}/live/ not accessible"
    echo ""
    echo "    To fix manually, run:"
    echo "      mount -o loop,ro ${ISO_DIR}/${ISO_NAME} ${ISO_CONTENTS}"
    echo "      wget -q --spider ${LIVE_SQUASHFS_URL} && echo OK"
fi
...
echo " Squashfs    : ${LIVE_SQUASHFS_URL}"
echo ""
